...
void GetDevices(const std::vector<cl::Platform>& platforms,
    ...
    for(const cl::Platform& pl: platforms) {
        ...
        cl_int err = pl.getDevices(type, &temp_devices);
        if (err != CL_SUCCESS) {
            LOGI("Can not get any devices of type: %d, error code: %d", (int32_t)type, err);
        }
...
void TryAdd() {
    std::string HelloWorldC =  "kernel void helloworld(global const int* in, global int* out) {\n"
    ...
    if (platforms.empty()) { LOGI("can not get any opencl platforms."); return; }
    // std::string platform_name = platforms[0].getInfo<CL_PLATFORM_NAME>();

    // get devices of GPU
    std::vector<cl::Device> devices;
    GetDevices(platforms, CL_DEVICE_TYPE_GPU, &devices);
    if (devices.empty()) { LOGI("can not get any GPU devices."); return; }
    ...
    sources.push_back({HelloWorldC.c_str(), HelloWorldC.length()});
    ...
    cl::Buffer outputBuffer = cl::Buffer(context, CL_MEM_WRITE_ONLY, NUM * sizeof(int), nullptr, &err);
    ...
    // run kernel function
    cl::Event event;
    err = queue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(NUM), cl::NullRange, nullptr, &event);
    // event.wait();
    queue.finish();

    /**Step 11: Read the cout put back to host memory.*/
    // read result, read the result put back to host memory
    queue.enqueueReadBuffer(outputBuffer, CL_TRUE, 0, NUM * sizeof(int), output, nullptr, nullptr);
...
std::string GetDevicesName() {
    ...
    for (const cl::Platform& pl: platforms) {
        ...
        for (const cl::Device& device : devices) {
            sz += "name ";
            sz += std::string(device.getInfo<CL_DEVICE_NAME>()); // device name
            sz += ", vendor ";
            sz += std::string(device.getInfo<CL_DEVICE_VENDOR>());
            sz += " | ";
        }
    }
